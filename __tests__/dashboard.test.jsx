...
describe("Dashboard Component", () => {
  ...
  it("render on mobile", async () => {
    ...
    const specificClassTables = tables.filter(table => table.classList.contains("sm:hidden"));
  ...
  it("render on desktop", async () => {
    ...
    const tables = screen.getAllByRole("table");
    // Check if the table has the correct class
    const specificClassTables = tables.filter(table => table.classList.contains("sm:table"));
    expect(specificClassTables.length).toBeGreaterThan(0);
  });
});
