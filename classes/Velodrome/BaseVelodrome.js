...
export class BaseVelodrome extends BaseProtocol {
  constructor(chain, chaindId, symbolList, mode, customParams) {
    ...
    this.protocolContract = getContract({
      client: THIRDWEB_CLIENT,
      address: this.customParams.routerAddress,
      chain: CHAIN_ID_TO_CHAIN[this.chainId],
      abi: Router,
    });
  ...
  async _calculateMintLP(tokenAmetadata, tokenBmetadata) {
    ...
    // Otherwise, use min((amount0 * totalSupply) / reserve0, (amount1 * totalSupply) / reserve1)
    const amount0Liquidity = tokenAmetadata.minAmount
      .mul(_totalSupply)
      .div(reserve0);
    const amount1Liquidity = tokenBmetadata.minAmount
      .mul(_totalSupply)
      .div(reserve1);

    return amount0Liquidity.lt(amount1Liquidity)
      ? amount0Liquidity
      : amount1Liquidity;
  }
